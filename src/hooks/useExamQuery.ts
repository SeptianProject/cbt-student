...
import { AssignedExam, ParsedQuestion, StudentAnswer, ExamSubmitOptions } from '@/types';
...
export const examKeys = {
     ...
     sessionStatus: (examId: number) => [...examKeys.all, 'sessionStatus', examId] as const,
     activeSession: (examId: number) => [...examKeys.all, 'activeSession', examId] as const,
};
...
export const useStartExamMutation = () => {
     ...
     return useMutation({
          ...
          onSuccess: (data, examId) => {
               ...
               queryClient.invalidateQueries({ queryKey: examKeys.sessionStatus(examId) });
          },
          onError: (error: Error) => {
...
export const useClearExamSessionMutation = () => {
     ...
     return useMutation({
          ...
          onSuccess: (_, examId) => {
               ...
               queryClient.invalidateQueries({ queryKey: examKeys.sessionStatus(examId) });
               queryClient.invalidateQueries({ queryKey: examKeys.activeSession(examId) });
          },
     });
};
...
export const useSubmitExamMutation = () => {
     ...
     return useMutation({
          ...
          onSuccess: (data, variables) => {
               ...
               queryClient.invalidateQueries({ queryKey: examKeys.sessionStatus(variables.examId) });
...
export const useCheckActiveSession = (examId: number, enabled: boolean = true) => {
     return useQuery({
