...
export const useAutoSaveAnswer = ({
  ...
  const saveAnswer = useCallback(
    async (questionId: number, answer: StudentAnswer) => {
      ...
      try {
        ...
        const answerType =
          question.question_type_id === "3" ? "essay" : "choice";
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Jawaban tidak tersimpan. Silakan periksa koneksi internet Anda.";
  ...
  useEffect(() => {
    ...
    if (changedAnswers.length > 0) {
      ...
      saveTimeoutRef.current = setTimeout(() => {
        // Save all changed answers sequentially to avoid request bursts.
        void (async () => {
          for (const { questionId, answer } of changedAnswers) {
