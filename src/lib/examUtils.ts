...
// Helper function to sanitize all values in a choices record (strip HTML)
const sanitizeChoicesValues = (
  record: Record<string, string>,
): Record<string, string> => {
  const sanitized: Record<string, string> = {};
  for (const key in record) {
    sanitized[key] = stripHtml(String(record[key]));
  }
  return sanitized;
};
...
const parseChoices = (choices: unknown): Record<string, string> => {
  // If already an object (not array), sanitize and return
  if (
    typeof choices === "object" &&
    choices !== null &&
    !Array.isArray(choices)
  ) {
    return sanitizeChoicesValues(choices as Record<string, string>);
  }
  ...
  if (typeof choices === "string") {
    ...
    if (Array.isArray(parsed)) {
      ...
      parsed.forEach((item, index) => {
        ...
        result[key] = stripHtml(String(item));
      });
      return result;
    }
    ...
    if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
      return sanitizeChoicesValues(parsed as Record<string, string>);
    }
    ...
    if (trimmed) {
      ...
      for (const pattern of patterns) {
        ...
        if (matches.length > 0) {
          ...
          matches.forEach((match) => {
            result[match[1].toUpperCase()] = stripHtml(match[2].trim());
          });
      ...
      if (items.length > 0) {
        ...
        items.forEach((item, index) => {
          const key = String.fromCharCode(65 + index); // A, B, C, D...
          result[key] = stripHtml(item);
        });
        return result;
      }

      // Last resort: single option
      return { A: stripHtml(trimmed) };
    }
  }
...
const parseChoicesImages = (
  choicesImages: unknown,
): Record<string, string | null> | null => {
  if (!choicesImages) return null;

  // If already an object (not array), return as is
  if (
    typeof choicesImages === "object" &&
    choicesImages !== null &&
    !Array.isArray(choicesImages)
  ) {
    ...
    switch (question.question_type_id) {
      case "0": // Single choice (Pilihan Ganda)
        if (
          ...
          validation.warnings.push(
            `Soal ${index + 1} (pilihan ganda) belum dijawab`,
          );
        }
        break;
      ...
      case "1": // Multiple choice complex (Pilihan Ganda Kompleks)
        if (!Array.isArray(answer.answer) || answer.answer.length === 0) {
          validation.warnings.push(
            `Soal ${index + 1} (pilihan ganda kompleks) belum dijawab`,
          );
        }
        break;
      ...
      case "2": // True/False (Benar Salah)
        if (
          ...
          validation.warnings.push(
            `Soal ${index + 1} (benar/salah) belum dijawab`,
          );
        }
        break;
      ...
      case "3": // Essay
        if (
          ...
          validation.warnings.push(
            `Soal ${index + 1} (essay) mungkin terlalu singkat`,
          );
        }
        break;
    }
