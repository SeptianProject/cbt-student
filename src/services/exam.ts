...
export const examService = {
  ...
  examStart: async (examId: number): Promise<Exam> => {
    ...
    try {
        ...
        if ("session_token" in response.data) {
          ...
          if (responseWithToken.session_token) {
            localStorage.setItem(
              "session_token",
              responseWithToken.session_token,
            );
          }
        }
        if ("session_id" in response.data) {
          const responseWithSessionId = response.data as {
            session_id?: number;
          };
      ...
      if (
        ...
        const retryResponse = await api.post<Exam>(
          `/siswa/exams/${examId}/start`,
          {},
          {
            headers: {
        ...
        if (retryResponse.data && typeof retryResponse.data === "object") {
          if ("session_token" in retryResponse.data) {
            const responseWithToken = retryResponse.data as {
          ...
          if ("session_id" in retryResponse.data) {
            const responseWithSessionId = retryResponse.data as {
              session_id?: number;
            };
            if (responseWithSessionId.session_id) {
              localStorage.setItem(
  ...
  submitExam: async (
    ...
    if (!sessionToken) {
      throw new Error(
        "Session token tidak ditemukan. Silakan mulai ulang ujian.",
      );
    }
    ...
    Object.values(answers).forEach((answer) => {
      ...
      if (
        ...
        if (question.question_type_id === "3") {
          ...
          if (essayAnswer.trim()) {
            essayAnswers[answer.question_id.toString()] = essayAnswer.substring(
              0,
              5000,
            );
          }
          ...
          if (mcAnswer.trim()) {
            multipleChoiceAnswers[answer.question_id.toString()] =
              mcAnswer.substring(0, 10);
          }
        }
      }
    });
    ...
    if (
      Object.keys(multipleChoiceAnswers).length === 0 &&
      Object.keys(essayAnswers).length === 0 &&
      !options.forceSubmit
    ) {
      throw new Error(
        "Tidak ada jawaban yang ditemukan. Silakan jawab minimal satu soal atau gunakan force submit.",
      );
    }
    ...
    try {
      ...
      if (error && typeof error === "object" && "response" in error) {
        ...
        if (axiosError.response?.status === 500) {
          throw new Error(
            "Terjadi kesalahan saat menyimpan hasil ujian. Silakan coba lagi.",
          );
        } else if (axiosError.response?.status === 405) {
          throw new Error(
            "Metode request tidak didukung. Silakan refresh halaman dan coba lagi.",
          );
        } else if (axiosError.response?.status === 404) {
          throw new Error(
            "Endpoint ujian tidak ditemukan. Silakan hubungi administrator.",
          );
        } else if (axiosError.response?.status === 422) {
          throw new Error(
            "Data yang dikirim tidak valid. Silakan periksa jawaban Anda.",
          );
  ...
  updateAnswer: async (
    ...
    try {
      const response = await api.post(
        "/siswa/exam-session/update-answer",
        payload,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("api_token")}`,
  ...
  getSavedAnswers: async (
    sessionId: number,
  ): Promise<Record<number, StudentAnswer>> => {
    ...
    try {
      ...
      const response = await api.get<
        GetSavedAnswersResponse | DirectResponseFormat
      >(`/siswa/exam-session/${sessionId}/answers`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("api_token")}`,
        },
      });
      ...
      const isWrappedFormat =
        "success" in response.data && response.data.success;
      const isDirectFormat =
        "session_id" in response.data && !("success" in response.data);
      ...
      if (
        essayAnswersData &&
        typeof essayAnswersData === "object" &&
        !Array.isArray(essayAnswersData)
      ) {
        Object.entries(essayAnswersData).forEach(
  ...
  getCompactAnswers: async (
    ...
    try {
      const response = await api.get<CompactAnswersResponse>(
        `/siswa/exam-session/${sessionId}/compact-answers`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("api_token")}`,
          },
      ...
      if (response.data?.success && response.data?.data?.answers) {
        ...
        Object.entries(answersObject).forEach(
          ([questionIdStr, answerValue]) => {
            const questionId = parseInt(questionIdStr);
            const answerStr = String(answerValue || "");
  ...
  getSessionProgress: async (
    sessionId: number,
  ): Promise<SessionProgressResponse["data"] | null> => {
    if (!sessionId || sessionId <= 0) {
      return null;
    }

    try {
      const response = await api.get<SessionProgressResponse>(
        `/siswa/exam-session/${sessionId}/progress`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("api_token")}`,
          },
      ...
      Object.values(answers).forEach((answer) => {
        ...
        if (
          ...
          if (question.question_type_id === "3") {
            ...
            if (essayAnswer.trim()) {
              essayAnswers[answer.question_id.toString()] =
                essayAnswer.substring(0, 5000);
            }
          } else {
  ...
  /**
   * Validate exam session status - check if session is still valid/active
   * Used to detect if session has been force-exited or locked
   * Endpoint: GET /siswa/exam-session/{sessionId}/validate
   */
  validateExamSessionStatus: async (
    sessionId: number,
  ): Promise<{
    success: boolean;
    is_valid: boolean;
    ...
    try {
      const response = await api.get(
        `/siswa/exam-session/${sessionId}/validate`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("api_token")}`,
          },
      ...
      if (
        ...
        return {
          success: false,
          is_valid: false,
          is_locked: response.data?.is_locked || true,
          message: response.data?.message || "Session is locked or invalid",
        };
      }
      ...
      console.error("❌ Failed to validate exam session status:", error);
      // On error, assume session is invalid for safety
      return {
        success: false,
        is_valid: false,
        is_locked: true,
